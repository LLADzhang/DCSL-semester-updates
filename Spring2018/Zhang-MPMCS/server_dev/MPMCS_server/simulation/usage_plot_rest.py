...
if not large:
    ...
    sa_minutes.extend([3,3,3,3,4,4])
    sa_minutes.extend( [4,4,4,5,5,5])
    sa_minutes.extend( [5,5,5,5,5,5])
    sa_minutes.extend([5,5,5,5,6,6])
    sa_minutes.extend([6,6,6,6,6,6])
    sa_minutes.extend([7,7,7,7,7,7])
    sa_minutes.extend([7,7,8,8,8,8])
    sa_minutes.extend([8,8,8,9,9,9])

    sa_minutes = [minute / 10 for minute in sa_minutes]
else:
    asa_minutes = [0,0,0,0,0,0]
    asa_minutes.extend([1,1,1,1,1,1])
    asa_minutes.extend([2,2,2,2,2,2])
    asa_minutes.extend( [2,2,2,2,2,2])
    asa_minutes.extend( [2,2,2,2,5,5])
    asa_minutes.extend([5,5,5,5,6,6])
    asa_minutes.extend([6,6,6,6,6,6])
    asa_minutes.extend([7,7,7,8,8,8])
    asa_minutes.extend([8,8,9,9,9,9])
    asa_minutes.extend([9,10,10, 10,10,10])
    ...
    sa_minutes = [0,0,0,1,1,1]
    sa_minutes.extend([1,1,2,2,2,2])
    sa_minutes.extend([3,3,3,3,3,3])
    sa_minutes.extend([3,3,3,4,4,4])
    sa_minutes.extend([4,4,4,4,4,4])
    sa_minutes.extend([4,4,4,4,4,4])
    sa_minutes.extend([4,4,4,4,4,4])
    sa_minutes.extend([4,4,4,4,4,4])
    sa_minutes.extend([4,4,4,4,4,4])
    sa_minutes.extend([4,4,4,4,5,5])
...
ax1.set_xlabel('Timestamp (seconds)', fontsize=15, labelpad=2)
ax1.set_ylabel('CPU Utilization', fontsize=15, labelpad=0)
#ax2.set_ylabel('Percentage of finished tasks', fontsize=14, labelpad=2)
ax1.legend(bbox_to_anchor=(-0.02, 0.5), loc='upper left' , fontsize=18)
#ax2.legend(bbox_to_anchor=(0, 0.7), loc = 'lower left', fontsize=12)
pyplot.show()
